...
const Login = () => {
  ...
  const plans = ['Plan Gratis Judicial', 'Plan Medio Judicial', 'Plan Pro Judicial'];
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegistering) {
        ...
        await supabase.from('profiles').insert([{ id: data.user?.id, full_name: fullName, status: 'Activo', plan: selectedPlan }]);
      } else {
  ...
  return (
    <DottedBackground className="min-h-screen w-full flex items-center justify-center p-0 m-0" style={{ backgroundColor: '#0f172a' }}>
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ width: '1000px', display: 'grid', gridTemplateColumns: '1fr 1fr', overflow: 'hidden', backgroundColor: 'rgba(255, 255, 255, 0.05)', borderRadius: '24px' }}>
        <div style={{ padding: '4rem', color: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h1 style={{ fontSize: '3rem', fontFamily: 'serif', marginBottom: '1.5rem' }}>GMA Dynamics</h1>
        ...
        <div style={{ padding: '4rem', backgroundColor: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {isRegistering && (
              <select value={selectedPlan} onChange={e => setSelectedPlan(e.target.value)} style={{ padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
            ...
            <button type="button" onClick={() => supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin } })} style={{ padding: '1rem', backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
